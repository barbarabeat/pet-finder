...
<div id="column">
    ...
    >
        ...
        <v-divider class="mx-4"></v-divider>
            ...
            <v-card-text>id: {{item.id}} - {{item.id}}</v-card-text>  
      <v-btn
        color="success"
        text
...
export default {
    ...
    methods: {
        ...
        async deletePet(index) {
            console.log(this.pets[index.id]);
            console.log(this.pets);    
            let toDelete;
            for(let i=0; i<=this.pets.length; i++){
                console.log(index[this.item.id]);
                if(this.pets[i].id == index[i])
                toDelete=this.pets[i].id;
            }
            try{ await api.delete(`/pets/${toDelete}`);
